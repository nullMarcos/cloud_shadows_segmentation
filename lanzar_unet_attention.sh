...
# 1. Alerta de Inicio
curl -s -H "Content-Type: application/json" -X POST -d '{"content": "🚀 **MethaneSAT:** Iniciando `run_experiment.py` con Unet-Attention..."}' "$URL" > /dev/null
...
python run_experiment.py --config config/msat_cs_unet_attention.yaml 2>&1 | tee experiment.log | while IFS= read -r line; do
